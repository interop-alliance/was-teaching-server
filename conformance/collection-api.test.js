...
describe('Collections API', () => {
  ...
  it('POST /space/:spaceId/ should 401 error when no authorization headers', async () => {
    const response = await fetch(new URL('/space/any-space-id/', serverUrl), { method: 'POST' })
  ...
  it('POST /space/:spaceId/ should 404 error on not found space id', async () => {
    ...
    let expectedError
    try {
      await alice.rootClient.request({ url: spaceUrl, method: 'POST', action: 'POST' })
    } catch (err) {
      expectedError = err
    }
  ...
  it('[root] create and delete a collection by id', async () => {
    ...
    const collectionUrl = new URL(
      `/space/${alice.space1.id}/${collectionId}`, serverUrl
    ).toString()
    const body = { id: collectionId, name: 'New Collection' }

    await alice.rootClient.request({ url: collectionUrl, method: 'PUT', json: body })
