 ...
 * never forces a rewrite. The wrap is `@interop/minimal-cipher`'s `createKek`
 * (RFC 3394 A256KW), the same primitive the KMS module's AES-KW key type uses.
 *
 ...
 * @returns {Promise<KmsKeyRecord>}   the at-rest record (secrets replaced by `encrypted`)
 */
export async function encryptKeyRecord({
  record,
  kek
}: {
  record: KmsKeyRecord
  ...
  const wrappedCek = await (
    await createKek({ keyData: kek.key })
  ).wrapKey({ unwrappedKey: cek })
 ...
 * @returns {Promise<KmsKeyRecord>}   the decrypted record (secrets restored on `key`)
 */
export async function decryptKeyRecord({
  ...
  const cek = await (
    await createKek({ keyData: kek.key })
  ).unwrapKey({ wrappedKey: jwe.recipients[0]!.encrypted_key })
  if (cek === null) {
    throw new Error(
      `KEK "${encrypted.kekId}" does not unwrap this WebKMS key record ` +
        '(RFC 3394 integrity check failed).'
    )
  }
